...
int	check(t_map *map, size_t *map_len)
{
	...
	while (map->map[y])
	{
		...
		while (map->map[y][x])
		{
			if (map->map[y][x] == '\n' || (ismap(map->map[y][x] && inmap(map->map[y][x]) && !(x && ismap(map->map[y][x - 1]) && map->map[y][x + 1] && ismap(map->map[y][x + 1]) && y && map_len[y - 1] >= x + 1 && ismap(map->map[y - 1][x - 1]) && ismap(map->map[y - 1][x]) && ismap(map->map[y - 1][x + 1]) && map->map[y + 1] && map_len[y + 1] >= x && ismap(map->map[y + 1][x - 1]) && ismap(map->map[y + 1][x]) && ismap(map->map[y + 1][x + 1])))))
			{
				// printf("%zu %zu\n", x, y);
				// printf("error\n");
				return (1);
