...
void			check_side(t_data *ptr)
{
	...
	{
		ptr->dda->stepx = -1;
		ptr->dda->sidedist_x = (ptr->pl->posX - ptr->dda->map_x) *
		ptr->dda->deltadist_x;
	}
	else
	{
		ptr->dda->stepx = 1;
		ptr->dda->sidedist_x = (ptr->dda->map_x + 1.0 - ptr->pl->posX) *
		ptr->dda->deltadist_x;
	}
	if (ptr->dda->raydir_y < 0)
	{
		ptr->dda->stepy = -1;
		ptr->dda->sidedist_y = (ptr->pl->posY - ptr->dda->map_y) *
		ptr->dda->deltadist_y;
	}
	else
	{
		ptr->dda->stepy = 1;
		ptr->dda->sidedist_y = (ptr->dda->map_y + 1.0 - ptr->pl->posY) *
		ptr->dda->deltadist_y;
	}
}
...
void    raycaster(t_data *data)
{
	...
	while (data->dda->screenx < screenWidth)
	{
		calc_dda(data);
		run_draw(data);
		// data->buffer[data->dda->screenx] = data->dda->walldist; // --> for sprite
		data->dda->screenx++;
	}
	update_fps(data);
	// draw_sprite(ptr);
